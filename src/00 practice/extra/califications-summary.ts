...
const summarizeClassRoom = (studentList: Student[]): StudentSummary[] => {
  const studentSummary = studentList.map((student) => {
    return {
      name: student.name,
      highestCalification: highest(student.califications),
...
const average = (numbers: number[]): number => {
  ...
  return Number((sum / numbers.length).toPrecision(3));
};

const highest = (numbers: number[]): number => {
  return numbers.reduce((acc, curr) => (acc >= curr ? acc : curr));
};

// Another way to get the highest number
const highest2 = (numbers: number[]): number => {
